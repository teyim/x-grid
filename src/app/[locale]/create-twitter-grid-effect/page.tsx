...
export async function generateMetadata({ params }: Props) {
  ...
  return buildLocalizedMetadata(locale as SeoLocale, 'effect');
}

export default async function LocalizedTwitterEffectPage({ params }: Props) {
  const { locale } = await params;
  if (!seoLocales.includes(locale as SeoLocale)) notFound();
  return <LocalizedToolPage locale={locale as SeoLocale} kind="effect" />;
}
