...
export async function generateMetadata({ params }: Props) {
  ...
  return buildLocalizedMetadata(locale as SeoLocale, 'instagram');
}

export default async function LocalizedInstagramPage({ params }: Props) {
  const { locale } = await params;
  if (!seoLocales.includes(locale as SeoLocale)) notFound();
  return <LocalizedToolPage locale={locale as SeoLocale} kind="instagram" />;
}
